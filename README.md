...
- **☁️ Supabase 雲端後端** ⚡ NEW!
  - 🔐 企業級 Email/密碼認證
  - 🛡️ Row Level Security 資料保護
  - 🗄️ PostgreSQL 雲端資料庫
  - 📸 雲端圖片儲存 (Supabase Storage)
  - 🔄 即時同步，無需手動部署
...
#### 管理後臺
- **登入**: `/admin`（使用 Supabase 帳號）
- **公告管理**: 即時新增/編輯公告
- **產品管理**: 上傳產品圖片與資訊
- **雲端同步**: 資料即時儲存到 Supabase
...
### 最新功能
- ✅ **Supabase 後端整合** - 雲端資料庫 + 認證 + 儲存
- ✅ **全新管理後台 UI** - 現代化設計，漸層背景
- ✅ **雲端/本地模式指示** - 清楚顯示資料儲存位置
- ✅ **即時資料同步** - 無需手動部署
...
│   ├── FooterSection.vue
│   ├── UploadSection.vue
│   ├── AdminPanelV2.vue     # 🆕 Supabase 管理後台
│   └── ui/                  # shadcn-vue 元件
├── views/                   # 頁面視圖
│   ├── HomeView.vue
